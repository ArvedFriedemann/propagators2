...
test1 :: IO ()
test1 = runTestSEB $ do
    sv_a <- fromVarsAsCells (Direct A) (ls [var $ Direct $ Sv 0, ccon "a"])
    b_sv <- fromVarsAsCells (Direct B) (ls [ccon "b", var $ Direct $ Sv 0])
    eq sv_a b_sv
    return [sv_a, b_sv, Direct (Sv 0)]

test2 :: IO ()
test2 = runTestSEB $ do
    t1 <- fromVarsAsCells (Direct A) (ls [var $ Direct $ Sv 1, ccon "a", var $ Direct $ Sv 1])
    t2 <- fromVarsAsCells (Direct B) (ls [var $ Direct $ Sv 2, ccon "a"])
    eq t1 t2
...
test3 = runTestSEB $ do
    ...
    t2 <- fromVarsAsCells (Direct B) (ls [ccon "a", var $ Direct $ Sv 0])
    eq t1 t2
    return [t1, t2, Direct $ Sv 0]

test4 :: IO ()
test4 = runTestSEB $ do
    orig <- fromVarsAsCells (Direct A) (ls [var $ Direct $ Sv 1, ccon "A"])
    t1 <- fromVarsAsCells (Direct B) (ls [ccon "B", ccon "A"])
    t2 <- fromVarsAsCells (Direct C) (ls [ccon "B", ccon "B"])
...
instance Identifier TC (S.Set TD)

test5 :: IO ()
test5 = flip runSEB (>> pure ()) $ do
    write (TC 2) ([TD_A, TD_C] :: S.Set TD)

    fork () $ \ lft -> do
        watch (TC 2) () $ lft . write Orig
