...
disjunctFork :: (PropagatorMonad m, Forkable m, BoundedMeet a, BoundedJoin a, Value a) => Cell m a -> m () -> m () -> m ()
...
disjunct :: (PropagatorMonad m, BoundedJoin a, Value a) => Cell m a -> Cell m a -> Cell m a -> m (Subscriptions m)
disjunct a b r = do
...
disjunctListener :: (PropagatorMonad m, BoundedJoin a, Value a) => Cell m a -> Cell m a -> a -> m ()
